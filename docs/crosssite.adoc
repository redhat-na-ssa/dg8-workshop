...
Open the link:{{ CONSOLE_URL }}[OpenShift web console^].

Choose the project `{{ USER_ID }}-cache`.

Create a new Data Grid instance by navigating to `Installed Operators > Data Grid > Create Infinispan ` as the following picture shows:

image::create-infinispan.png[Cross site - Topology, 700]

Press the button `Create Infinispan`.

First, we will create a new Infinispan cluster for our NYC location in the `{{ USER_ID }}-cache` project.
...
Let's do the same for our LON site in the project `{{ USER_ID }}-cache2`:
...
echo "https://$(oc get route example-lon-external -o jsonpath='{.spec.host}')"
...
After pressing `Create cache` you will a form. Enter `xsiteCache` as the cache name, and select `Add cache configuration` and click on the `next` button.
On the next form, add the following XML and press `create`:
...
echo "https://$(oc get route example-nyc-external -o jsonpath='{.spec.host}')"
