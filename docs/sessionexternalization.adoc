...
Now we will build and deploy the project using the following command, which will use the maven plugin to deploy via Dev Spaces Terminal:

[source,sh,role="copypaste"]
----
mvn clean package spring-boot:repackage -f $PROJECTS_ROOT/dg8-workshop-labs/dg8-spring-session
----
...
[source,sh,role="copypaste"]
----
oc start-build cacheapp --from-file=$PROJECTS_ROOT/dg8-workshop-labs/dg8-spring-session/target/rhdg-0.0.1-SNAPSHOT.jar --follow
----
