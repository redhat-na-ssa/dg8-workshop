...
In this workshop, we will use Red Hat Dev Spaces, an online IDE based on https://www.eclipse.org/che/[Eclipe Che, window=_blank]. This product has several benefits:
...
To get started, access {{ CHE_URL }}[the Dev Spaces instance^] and log in using the username and password you are assigned (e.g. `{{ USER_ID }}/{{ OPENSHIFT_USER_PASSWORD }}`):

image::devspace-login.png[devspace-login, 700, align="center"]

Using the Devspaces dashboard, let's create a workspace for this workshop.
In the `Import from Git` section, copy the following Github repository link in the `Git repo URL` field. Click `Create & Open` to create and start the workspace.

[source, shell, role="copypaste"]
----
https://raw.githubusercontent.com/redhat-na-ssa/dg8-workshop-labs/refs/heads/main/devfile.yml
----

image::devspace-dashboard.png[devspace-dashboard, 700, align="center"]

This process will analyze the Git repository to check if there is any definition on how this environment should be created. This definition is the Devfile.

[NOTE]
====
Since this is the first time you are using this IDE, some things need to be configured, like the VS Code extensions defined for this environment. Accept and Trust the configuration to finalize the start up process.
====

image::devspace-loading.png[devspace-loading, 700, align="center"]
...
image::devspaces-view-exercises.png[devspaces-view-exercises, 700, align="center"]
