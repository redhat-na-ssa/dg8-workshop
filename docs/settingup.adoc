...
* https://developers.redhat.com/products/codeready-workspaces/overview[Red Hat Dev Spaces Workspaces,window=_blank] - based on *Eclipse Che*, it’s a cloud-based, in-browser IDE (similar to IntelliJ IDEA, VSCode, Eclipse IDE). You’ve been provisioned your own personal workspace for use with this workshop. You’ll write, test, and deploy code from here.
