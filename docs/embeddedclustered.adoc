...
Now open another terminal in Dev Spaces workspaces and change to the scripts directory
...
in this directory we have a load.sh file. Open this file in Dev Spaces Workspaces and change the variable `EP` to the application route from the browser (including the `/api` suffix):
