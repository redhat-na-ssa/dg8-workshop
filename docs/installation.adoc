...
First, you need access to the cluster: 

Open a new tab with the link:{{ CONSOLE_URL }}[OpenShift web console^]. You will be prompted to log in. Use the following credentials:
...
Now let's retrieve the Data Grid CR definition from the command line. To do this, head over to the Dev Spaces Workspace and log in OpenShift. 

image::devspaces_terminal.gif[openshift_login, 700]

You should already be logged in. You can confirm your user by running `oc whoami` in the console. It should match your assigned user. 

If you are not, just run `oc login https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_SERVICE_PORT --insecure-skip-tls-verify=true --username={{ USER_ID }} --password={{ OPENSHIFT_USER_PASSWORD }}`.
...
image::dg_edit_CR_Route.png[Edit and Save, 700]

Perfect. Now press `save`.

<1> Navigate back to the `Administrator` view.
<2> Then click on `Networking > Routes` 
you should see the following page with the address for the DataGrid Console

image::dg_routes.png[DG cluster detail view, 700]


Click on the link to open the terminal:

[NOTE]
====
Because we are using self-signed certificates, you will need to manually accept them to access the console. 
====

To be able to access the console, you will need to provide credentials. 

The Data Grid operator creates the credentials during installation time and they should be stored in your namespace secrets. Head back to your Dev Spaces Workspace terminal. Let's get the secret with the following command.
