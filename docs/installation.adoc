...
* *STEP 1*: If you have not already logged in OpenShift from the Dev Spaces Workspaces terminal, please do that now. Click on the `Login to Openshift` menu in the right menu called 'Workspace'. When prompted, your *Username* is `{{ USER_ID }}` and your *Password* `{{ OPENSHIFT_USER_PASSWORD }}`.
...
Now let's retrieve the Data Grid CR definition from the command line. To do this, head over to the Dev Spaces Workspace and log in to Openshift. 

image::devspaces_terminal.gif[openshift_login, 700]


NOTE: You should already be logged in, but if you are not, just run `oc login https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_SERVICE_PORT --insecure-skip-tls-verify=true --username={{ USER_ID }} --password={{ OPENSHIFT_USER_PASSWORD }}`
...
Initiate a `>_ New Terminal` in Dev Spaces workspace as in the previous section and run the following command
